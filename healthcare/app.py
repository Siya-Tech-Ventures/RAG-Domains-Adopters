...
with st.sidebar:
    ...
    if st.button("Initialize Systems"):
        if not st.session_state.api_key_valid:
            ...
            try:
                st.session_state.rag_system = HealthcareRAG(processed_docs_dir)
...
if not st.session_state.api_key_valid:
    ...
    # Create tabs
    tab1, tab2, tab3 = st.tabs(["Patient Care", "Documentation Query", "Upload Files"])
    
    # Patient Care Tab
    with tab1:
        st.header("Patient Care")
        if st.session_state.rag_system:
            query = st.text_input("Enter your patient care query:")
            if query:
                try:
                    response = st.session_state.rag_system.query_documentation(query, "patient_care")
                    st.write(response)
                except Exception as e:
    ...
    with tab3:
        ...
        category = st.selectbox(
            ...
            ["medical_records", "clinical_trials", "treatment_protocols", "patient_care"]
        )
...
# Footer
st.markdown("---")
st.markdown("Healthcare RAG System - Powered by OpenAI and LangChain")